...
window.Win10 = {
    ...
    _color:["black-green","green","blue","orange","red","dark","purple","blue-x","yellow","red-x"],
    _countTask: 0,
    _newMsgCount:0,
    _animated_classes:[],
    ...
    _init:function () {
        ...
        Win10.setContextMenu('#win10>.desktop',[
            ...
            ['<i class="fa fa-desktop"></i> '+Win10.lang('切换传统主题','admin'),function () {Win10.goAdmin()}]
        ]);
    ...
    setMenu:function(object){
        var color = this._color;
        var url = object.url;
        var dom = object.dom;
        $.get(url,function (returnData) {
            var datas =eval(returnData.data);
            var html = '';
            for(var key in  datas){
                var data = datas[key];
                if(data.url == '#'){
                    html = html + ' \n  <div  class="item" >\n' +
                        '                <i class="orange1 icon fa fa-folder-o fa-fw "></i>\n' +
                                  data.title+'</div>\n'  ;
                    ...
                    if (childrens) {
                        for(var j in  childrens){
                            ...
                            if (!children.icon){
                                children.icon = 'fa fa-file-o';
                            }
                            html = html + '<div class="sub-item"  onclick=\"'+'Win10.openUrl(\''+children.url+'\',\''+children.title+'\')\">\n' +
                                '                <i class="'+color[Math.floor((Math.random()*color.length))]+' icon ' + children.icon + ' fa-fw "></i>\n' +
                                children.title + '</div>\n';
                        }
                    }

                }else{
                    html = html +'\n<div class="item" onclick=\"'+'Win10.openUrl(\''+data.url+'\',\''+data.title+'\')\">\n' +
                        '                <i class="'+color[Math.floor((Math.random()*color.length))]+' icon '+data.icon+' fa-fw"></i>\n' +
                             data.title+'</div>\n' ;
                }
            ...
            for(var key in  datas){
                ...
                if(data.url == '#'){
                    html = html + ' \n  <div class="shortcut win10-drawer" >\n' +
                        '                <i class="orange1 icon fa fa-folder-o"></i>\n' +
                    ...
                    if (childrens) {
                        for(var j in  childrens){
                            ...
                            html = html + '<div class="shortcut-drawer win10-open-window" data-url="' + children.url + '">\n' +
                                '                <i class="'+color[Math.floor((Math.random()*color.length))]+' icon ' + children.icon + ' "></i>\n' +
                    ...
                    html = html +'\n<div class="shortcut win10-open-window" data-url="'+data.url +'">\n' +
                        '                <i class="'+color[Math.floor((Math.random()*color.length))]+' icon '+data.icon+' "></i>\n' +
                        '                <div class="title">'+data.title+'</div>\n' +
                        '            </div>\n'
                }
    ...
    exit:function () {
        layer.confirm(Win10.lang('确认要退出吗?','Are you sure you want to close this page?'), {icon: 3, title:Win10.lang('提示','Prompt')}, function(index){
    ...
    aboutUs: function() {
        ...
        layer.open({
            ...
            '<p>nonelonely©版权所有</p>' +
            '<p>作者邮箱:1343453657@qq.com</p>' +
            '</div>'
        });
    },
    goAdmin: function(){
        $.get("/system/main/changeAdmin?value=2",function (res) {
            if (res.code==200){
